...
{
  ConstExprParser::ConstExprParser(std::vector<Token> tokens, size_t startCursor) : tokens_(std::move(tokens)), cursor_(startCursor)
  {
  ...
  auto ConstExprParser::parsePrimary() -> ConstExprPtr
  {
    ...
    {
      static_cast<void>(consume());
      ConstExprPtr inner = parseAdditive();
