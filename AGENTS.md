...
- `src/driver.cpp` — `ngi` frontend: load → resolve → check → lower → compile → run, with registered natives
- `include/` — public vNext headers (`hir.hpp`, `typecheck.hpp`, `flowir.hpp`, `bytecode.hpp`, `value.hpp`, `syntax/*.hpp`, `driver.hpp`, ...)
- `example/*.ng` — runnable vNext examples; `lib/std/*.ng` — the vNext stdlib (prelude/io/string/seq/list/memory)
