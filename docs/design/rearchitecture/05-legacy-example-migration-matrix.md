...
| 10 | Variadic type/value packs, ranges/slices, folds | `49`, `54`, `57`, `58`, `59` | Heterogeneous variadic type packs (`T...` parameters/returns, tuple-literal spreads, per-instance tuple lowering) call-site tuple spreads with static flattening, and `range<i64>` values with checked array slicing are supported (`example/vnext/variadic_packs.ng`, `example/vnext/ranges_slicing.ng`); array spreads/folds, range iteration/comprehensions, and `sizeof_pack`/`tuple_element` remain. | R4–R7 |
