...
| 8 | Restricted const execution and `const if` | `17`, `42`, `46`, `47`, `53` | `const if` folding, module-level `const` predicates with pattern specialization (exact/pattern/primary priority, repeated parameters, `= delete`), prefix `ref<T>` sugar, and `const fun` (typed-HIR interpreter with loops, recursion, tail recursion, and runtime callability) are supported (`example/vnext/const_predicates.ng`, `example/vnext/const_fun.ng`); `= native` hosts, where clauses, generic const fun compile-time calls, and per-instance `const if` remain. | R5 |
