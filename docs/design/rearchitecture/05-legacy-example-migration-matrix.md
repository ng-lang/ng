...
| Arithmetic, comparison, prefix, logical, bitwise | `03`, `10`, `58` | Supported across the fixed-width integer tower (`i8`–`i64`, `u8`–`u64`) and `f32`/`f64` with contextual literal typing, numeric suffixes, per-width range checks, and cross-width numeric equality/ordering (`example/vnext/numeric_types.ng`, `example/vnext/floats.ng`) | Runtime per-width overflow checks and `isize`/`usize` remain. |
