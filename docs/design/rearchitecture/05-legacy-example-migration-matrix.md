...
| 14 | FFI and opaque/native handles | `39`, `45`, `51` | `native fun` runtime intrinsics with a registry and `print`/`assert` builtins (`example/native_io.ng`), opaque type declarations (`type X = native;` native handles, `type X;` abstract types) with built-in `is_trait<T>`/`is_abstract<T>` const predicates (`example/opaque_types.ng`); declared C ABI (`extern "C"`) and native handle lifecycle policies remain. | R9 |
