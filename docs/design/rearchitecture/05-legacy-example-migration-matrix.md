...
| 8 | Restricted const execution and `const if` | `17`, `42`, `46`, `47`, `53` | `const if` folding, module-level `const` predicates with pattern specialization (exact/pattern/primary priority, repeated parameters, `= delete`), and prefix `ref<T>` sugar are supported (`example/vnext/const_predicates.ng`); `= native` hosts, `const fun`, where clauses, and per-instance `const if` remain. | R5 |
