...
| R5 | Const evaluator, ownership analysis, and FlowIR | In progress — scalar FlowIR CFG and verifier; const expression parser, checked `ConstEvaluator`/`ConstInterner`, const generic parameter declarations, const-generic call instantiation, non-generic `const if`, and ref/raw-pointer type resolution (D-015 type layer) implemented; ownership/copy-move analysis, per-instance `const if`, and instance descriptor reuse remain pending | R4 | Typed compile-time evaluation and CFG dataflow |
