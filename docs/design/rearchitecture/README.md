...
| R5 | Const evaluator, ownership analysis, and FlowIR | In progress — scalar FlowIR CFG and verifier; const expression parser (with comparisons/bool literals), checked `ConstEvaluator`/`ConstInterner`, const generic parameter declarations, const-generic call instantiation, non-generic `const if`, module-level `const` declarations with pattern specialization (exact/pattern/primary priority, `= delete`), ref/raw-pointer type resolution, scoped reference place lowering (`ref`/`ref mut`/`*` with cell-backed bindings and deep-copy semantics), and enum `switch` dispatch lowering implemented; move/drop analysis, `const fun`, where clauses, per-instance `const if`, and instance descriptor reuse remain pending | R4 | Typed compile-time evaluation and CFG dataflow |
