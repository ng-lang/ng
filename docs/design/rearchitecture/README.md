...
| R3 | CompilationSession and module graph | In progress — vNext module name-resolution seed; transitive `import` loading with merged compilation units, deterministic cycle/missing-module diagnostics, and name-privacy enforcement (per-module visible-name sets with selective imports, export gating, and transitive re-export) | R1 | Session-scoped resolver/artifact cache |
...
| R5 | Const evaluator, ownership analysis, and FlowIR | In progress — scalar FlowIR CFG and verifier; const expression parser (with comparisons/bool literals), checked `ConstEvaluator`/`ConstInterner`, const generic parameter declarations, const-generic call instantiation (inferred and explicit), non-generic `const if`, module-level `const` declarations with pattern specialization (exact/pattern/primary priority, `= delete`), a typed-HIR `ConstInterpreter` for `const fun` (locals, if/const if, return, loop/next, tail recursion, nested const calls, fuel/recursion budgets), ref/raw-pointer type resolution, scoped reference place lowering (`ref`/`ref mut`/`*` with cell-backed bindings and deep-copy semantics), enum `switch` dispatch lowering, per-instance `where` clauses (predicates, `T is Type`, negation/`&&`, const fun calls over const parameters), heterogeneous variadic type packs (`T...` parameters, tuple-literal spreads, `tuple<T...>` returns with per-instance tuple lowering), tuple-splice bytecode, and traits Stage 1 (declarations, impls with coherence, static dispatch on concrete receivers, supertraits, default methods, qualified calls, `T: Trait` bounds) implemented; monomorphized generic instances (per concrete call substitution with re-checked bodies, covering method calls through bounded type parameters); `move`/`clone` expressions with affine use-after-move and partial-move tracking (branch/loop/switch merging, assignment revival), `impl Drop` lifecycle contracts (drop calls emitted at returns and fall-through for live affine values, skipped for moved-from bindings), simple borrow conflict checks with escaped-reference rejection (shared/mut borrow exclusivity per binding, block-scoped release, refs rejected in returns/aggregates), and array map comprehensions (`[f(xs)...]` runtime loop lowering with array length/append operations) implemented, plus fold calls (`f(acc, xs...)` left folds / `f(xs..., acc)` right folds over arrays, slices, and ranges, lowered to runtime loops with accumulator threading), and generic structs (`struct Box<T>`, per-instance field types) and higher-kinded `F<_>` type-constructor parameters with `F<T>` applications, built-in tuple/pack introspection (`is_tuple`/`tuple_size`/`sizeof_pack` const predicates plus `tuple_element`/`tuple_concat` type constructors) implemented; dynamic `ref<Trait>` views, field-aware partial-move drops, block-scoped drop edges, instance descriptor reuse remains pending | R4 | Typed compile-time evaluation and CFG dataflow |
...
| R9 | Native ABI, C FFI, opaque types, and bindgen | In progress — `native fun` runtime intrinsics with a session-owned registry, `print`/`assert` builtins, vNext stdlib io/string natives, opaque type declarations (`type X = native;` / `type X;`) with built-in `is_trait`/`is_abstract` const predicates, auto traits with `derive(Copy + Clone)`, a redesigned Dear ImGui binding over the SDL3 GPU backend (`lib/std/imgui.ng` + `src/imgui_natives.cpp`, `ngi_imgui` frontend), a self-hosting `runNgi` native with `--fuel 0` unlimited budgets, and a minimal NG IDE on the binding (`example/ng_ide.ng`, CI-covered via headless stub natives) | R4, R6, R7 | Declared ABI descriptors and safe C boundary |
