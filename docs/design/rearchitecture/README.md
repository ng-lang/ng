...
| R5 | Const evaluator, ownership analysis, and FlowIR | In progress — scalar FlowIR CFG and verifier; const expression parser, checked `ConstEvaluator`/`ConstInterner`, const generic parameter declarations, const-generic call instantiation, and non-generic `const if` implemented; ownership analysis, per-instance `const if`, and instance descriptor reuse remain pending | R4 | Typed compile-time evaluation and CFG dataflow |
