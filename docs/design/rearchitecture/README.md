...
| R5 | Const evaluator, ownership analysis, and FlowIR | In progress — scalar FlowIR CFG and verifier; const expression parser, checked `ConstEvaluator`/`ConstInterner`, const generic parameter declarations, and const-generic call instantiation implemented; ownership analysis, `const if` instance selection, and instance descriptor reuse remain pending | R4 | Typed compile-time evaluation and CFG dataflow |
