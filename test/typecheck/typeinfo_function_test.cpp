...
inline auto make_function(typeinfo_tag returnTypeTag, Vec<typeinfo_tag> paramTypeTags,
                          Vec<typeinfo_tag> paramWithDefault = {}) -> FunctionType
{
  ...
  std::transform(paramTypeTags.begin(), paramTypeTags.end(), std::back_inserter(parameterTypes),
                 [](auto tag) { return makecheck<PrimitiveType>(tag); });

  if (!paramWithDefault.empty())
  {
    std::transform(paramWithDefault.begin(), paramWithDefault.end(), std::back_inserter(parameterTypes),
                   [](auto tag) { return makecheck<ParamWithDefaultValueType>(makecheck<PrimitiveType>(tag)); });
  }
...
TEST_CASE("make_function works", "[TestUtil][TypeCheck][Function]")
{
  ...
  REQUIRE(make_function(typeinfo_tag::UNIT, {typeinfo_tag::I32}, {typeinfo_tag::I32}).repr() ==
          "fun (i32, i32 = default) -> unit");
  REQUIRE(make_function(typeinfo_tag::UNIT, {}, {typeinfo_tag::I32, typeinfo_tag::I32}).repr() ==
          "fun (i32 = default, i32 = default) -> unit");
}
...
TEST_CASE("test FunctionType#match", "[TypeCheck][Function][Covariance]")
{
  ...
  // fun (i16, i16) -> i32;
  auto narrower_params = make_function(typeinfo_tag::I32, {typeinfo_tag::I16, typeinfo_tag::I16});

  REQUIRE(narrower_params.match(basic_function));
  REQUIRE(!basic_function.match(narrower_params));

  // fun (i64, i64) -> i32;
  auto wider_params = make_function(typeinfo_tag::I32, {typeinfo_tag::I64, typeinfo_tag::I64});
