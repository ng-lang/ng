...
TEST_CASE("vNext ref example files run end to end through ngi", "[vNext][Ref][Examples]")
{
  std::string output;
  std::string errors;
  REQUIRE(runExample("example/vnext/ref_swap.ng", output, errors) == 0);
  INFO("errors: " << errors);
  REQUIRE(errors.empty());
  REQUIRE(output.find("with value 21") != std::string::npos);

  REQUIRE(runExample("example/vnext/ref_places.ng", output, errors) == 0);
  INFO("errors: " << errors);
  REQUIRE(errors.empty());
...
{
  const auto original = NG::vnext::Value::array({NG::vnext::Value::integer(1), NG::vnext::Value::integer(2)});
  auto copied = original.deepCopy();
  copied.asArrayMut()[0] = NG::vnext::Value::integer(9);
  REQUIRE(original.asArray()[0] == 1);
  REQUIRE(copied.asArray()[0] == 9);

  auto cell = std::make_shared<NG::vnext::Value>(NG::vnext::Value::integer(3));
  const auto first = NG::vnext::Value::reference(cell, {}, true);
